...
openvswitch-add-port:
  cmd.run:
    ...
    - unless: ovs-vsctl show |grep {{ salt['pillar.get']('vm_nic') }}
    - user: root
    - require:
      - cmd: set-vmnic-up
...
{% for log in ['openvwitch-agent','dhcp-agent','dhcp-helper'] %}
...
{% for srv in ['neutron-openvswitch-agent','neutron-dhcp-helper-agent'] %}
{{ srv }}:
  service.running:
    - enable: True
    - require:
      - file: /etc/neutron/plugin.init
